...
export const ConnectBtn = ({ isConnected, handleConnect, handleClose }) => {
  ...
  return (
    <button onClick={async () => await handleConnect()} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Connect</button>
  )
}
...
export const SyncBtn = ({ isConnected, handleSync }) => {
  return (
    <button disabled={!isConnected} onClick={() => handleSync()}
